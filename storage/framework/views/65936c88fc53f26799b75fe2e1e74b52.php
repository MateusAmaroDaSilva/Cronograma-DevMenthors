    ...
    <div class="flex-1 overflow-y-auto px-5 py-4 space-y-5">
        ...
        <div>
            <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
                Professores (<?php echo e($people->where('type', 'professor')->count()); ?>)
            </h3>
            <div class="flex flex-wrap gap-1.5">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $people->where('type', 'professor')->sortBy('name'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $person): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div draggable="true"
                         x-data="{ dragging: false }"
                         @dragstart="dragging = true; draggedPersonId = '<?php echo e($person->id); ?>'; draggedPersonType = 'professor'; $event.dataTransfer.setData('personId', '<?php echo e($person->id); ?>'); $event.dataTransfer.effectAllowed = 'move';"
        ...
        <div>
            <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
                Mentorados (<?php echo e($people->where('type', 'mentor')->count()); ?>)
            </h3>
            <div class="flex flex-wrap gap-1.5">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $people->where('type', 'mentor')->sortBy('name'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $person): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div draggable="true"
                         x-data="{ dragging: false }"
                         @dragstart="dragging = true; draggedPersonId = '<?php echo e($person->id); ?>'; draggedPersonType = 'mentor'; $event.dataTransfer.setData('personId', '<?php echo e($person->id); ?>'); $event.dataTransfer.effectAllowed = 'move';"
                         @dragend="dragging = false; draggedPersonId = null; draggedPersonType = null;"
                         :class="{ 'opacity-40 scale-95': dragging }"
                         class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium cursor-grab active:cursor-grabbing
...
<?php /**PATH C:\Users\Mateus Amaro\Documents\DevMenthors\Cronograma de Aulas\resources\views/schedule/sidebar.blade.php ENDPATH**/ ?>